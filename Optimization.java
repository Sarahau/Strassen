...
public class Optimization {
    final static int CROSSOVER_SIZE = 16;
    //Main Driver Code
    ...
    //Strassens recursive method where 7 submatrices are created and multiplied out
    ...
    public static Vector<Vector<Double>> Square_Matrix_Multiply(Vector<Vector<Double>> myDataA, Vector<Vector<Double>> myDataB) {
        int n = myDataA.size();
        Vector<Vector<Double>> myDataC = new Vector<Vector<Double>>();
        for(int i = 0; i < myDataA.size(); i++){
            for(int j = 1; j < myDataB.get(1).size(); j++){
                myDataC.elementAt(i).setElementAt(0.0, j);
                for(int k = 0; k < myDataA.get(1).size(); k++){
                    myDataC.elementAt(i).setElementAt(myDataC.elementAt(i).elementAt(j) + (myDataA.elementAt(i).get(k) * myDataB.elementAt(k).get(j)),j);
                }
            }
        }
        return myDataC;
